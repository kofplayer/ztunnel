...
### 已完成（2026-09-13，4 个提交，本地领先 origin/main，未推送）

`c96344c` 仓库卫生与 CI · `09b5ca3` 生命周期/加密/健壮性修复 · `1ecbb1a` 补齐测试覆盖 · `9a03f90` 修一处修复引入的数据竞争。

- 全部 **7 个 Critical** 与 8 个 High 中的 5 个已修（详见执行记录）
- 覆盖率 74.7% → 87.7%（语句，`-coverpkg=./...`），生产代码函数均值 92.6%，测试函数 48 → 215
- `go build` / `go vet` / `gofmt -l .` / `go test -race -count=2 ./...` 全绿；真实二进制端到端冒烟通过
...
| P2 | M-04 半关闭（需新增 `Conn.CloseWrite`，**目前不存在**）；M-16 发送侧就地改写；M-17 队列无增长上限；M-12 `netClient` 无状态守卫；M-14 后半（慢 `onAccept` 头阻塞 accept 循环）；M-03 剩余（ID 回绕覆盖） |
| P3 | L-2/L-3/L-10/L-11、E-06 改名、M-22 剩余（token 自描述编码 + 协议版本字节）、Phase 4 性能项（含一行即改的 `TCP_NODELAY`）、ws/ 注释代码清理、CI lint 转阻塞 |
