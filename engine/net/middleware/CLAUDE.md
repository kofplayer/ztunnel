...
- `NetMiddlewareFirst`（`first.go`）：链头。`SendData` 直连 socket 发送函数；`FireEvent` 带重入事件队列（握手期间在事件回调里再触发事件不会死锁/丢事件）。
...
1. **client OnConnect**：生成 `Key1` → 查表加密 8B → 发送。
...
4. **server WaitCsNo**：RSA 解出 CsNo → 生成 `ScNo`（server→client 方向）→ 用 `Key1^Key2^CsNo` 掩码后发送 → **FireEvent(OnReady)**。
...
⚠️ 已知取舍/风险：RSA-1024 强度弱（2026 年标准下不推荐，但零依赖+轻量场景的主动取舍）；XOR 掩码流密码强度有限——改动加密相关代码时**不要破坏握手报文的字节布局**（步骤 2/4 的掩码字段顺序），否则新旧版本不兼容。
...
3. 放在链中的位置决定语义：靠 First = 传输层职责（分包），靠 Last = 应用层职责（校验/业务编码）。
4. 需要就绪信号（如自定义握手）时，完成时 `FireEvent(MiddlewareEventOnReady)`，参考 type1。
